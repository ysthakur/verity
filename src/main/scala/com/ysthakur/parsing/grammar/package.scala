...
package object grammar {
    ...
    implicit class State(name: String) {
        def :=[Input, Helper](patternCases: PatternCase[Input, Helper]*)
                             (implicit tokenizer: LexerOrParser[_, _, _]): Unit =
            tokenizer.addStateCase(StateCase(name, patternCases.asInstanceOf[Iterable[PatternCase[tokenizer.I, tokenizer.Helper]]]))
    }
