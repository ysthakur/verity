...
trait TypeDef extends NamedTree {
  ...
  def makeRef: ResolvedTypeRef =
    makeRef(Text(this.name) :: Nil, TypeArgList(typeParams.params.map(_.makeRef), TextRange.synthetic))
  ...
  def strictSubTypeDefOf(sup: TypeDef): Boolean =
    this != sup && superTypes.exists {
      ...
      case unresolved.UnresolvedTypeRef(_, _, typeDef) => typeDef.fold(false)(_.strictSubTypeDefOf(sup))
    }
}
