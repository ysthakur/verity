...
//  def endOffset: Int = expr.endOffset
  override def text: StringBuilder = new StringBuilder(op.text) append expr.text
}
...
//  def endOffset: Int = op.endOffset
  override def text: StringBuilder = new StringBuilder(expr.text.toString) append op.text
}