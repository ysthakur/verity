...
case class Package(
  ...
  def findType(name: String): Option[TypeDef] =
    files.view.flatMap(_.typedefs).find(_.name == name)
}

object Package {

  /** Find a subpackage given the relative path of the package. The last found
    * subpackage and the remaining path are returned.
    * @param pkgPath
    *   The path of a subpackage or class or other tree somewhere inside this
    *   package. The Iterable is *not* in reverse order.
    */
  ...
  /** Find a subpackage given the absolute path of the package. The last found
    * subpackage and the remaining path are returned.
    * @param pkgPath
    *   The path of a subpackage or class or other tree somewhere inside this
    *   package. The Iterable is *not* in reverse order.
    */
  def findPkgPAbs(pkgPath: Iterable[String])(using
    root: Package
  ): (Package, Iterable[String]) =
    findPkgRel(root, pkgPath)

  def findImptableAbs(path: Iterable[String])(using
    root: Package
  ): Option[Def] =
